...
pub fn init(context: *AppContext) !void {
    ...
    var heading = try panel.add(allocator, try engine.create_label(
        allocator,
        display,
        .{
    ...
    try display.add_paragraph(allocator, scroller, .normal, "p1", "By using this app, you consent to this privacy policy and the terms of service.");
    try display.add_paragraph(allocator, scroller, .normal, "p2", "This app does not collect any personal identifying information or usage information.");
    try display.add_paragraph(allocator, scroller, .normal, "p3", "This app does not share any personal or usage information off your device.");
    try display.add_paragraph(allocator, scroller, .normal, "p4", "If new features are added that collect feedback or other types of information, it will be optional, and only be done after requesting your consent.");
    try display.add_paragraph(allocator, scroller, .normal, "p5", "Basic information about downloads or purchases through app stores are recorded by the app store provider and is used for accounting purposes.");
    try display.add_paragraph(allocator, scroller, .normal, "p6", "Questions about this privacy policy may be directed to the official social media accounts for this app.");
}
