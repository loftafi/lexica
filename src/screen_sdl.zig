...
pub fn init(context: *AppContext) !void {
    ...
    try display.add_paragraph(allocator, scroller, .normal, "p1", "SDL 3 is used under the zlib license:");
    try display.add_paragraph(allocator, scroller, .normal, "p2", "This software is provided 'as-is', without any express or implied warranty.  In no event will the authors be held liable for any damages arising from the use of this software.");
    try display.add_paragraph(allocator, scroller, .normal, "p3", "Permission is granted to anyone to use this software for any purpose, including commercial applications, and to alter it and redistribute it freely, subject to the following restrictions:");
    try display.add_paragraph(allocator, scroller, .normal, "p4", "1. The origin of this software must not be misrepresented; you must not claim that you wrote the original software. If you use this software in a product, an acknowledgment in the product documentation would be appreciated but is not required.");
    try display.add_paragraph(allocator, scroller, .normal, "p5", "2. Altered source versions must be plainly marked as such, and must not be misrepresented as being the original software.");
    try display.add_paragraph(allocator, scroller, .normal, "p6", "3. This notice may not be removed or altered from any source distribution.");
}
